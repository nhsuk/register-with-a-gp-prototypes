...
// V2 prototype. Sprints 3-. Recreation of GMS1 with GP lookup
...
router.get('/contact-details', function (req, res) {
  res.render('v1_1/contact-details', {
    session: req.session
  });
});
...
router.get('/previous-address-postcode', function (req, res) {
  res.render('v1_1/previous-address-postcode', {
    session: req.session
  });
});
        ...
        if (response.statusCode == 200) {
          ...
          if (req.session.prevpostcode !== '') {
            ...
            if (filtered.length === 0) {
              ...
              res.render('v1_1/previous-address-result', {
...
router.get('/select-previous-address', function (req, res) {
  res.render('v1_1/select-previous-address', {
    session: req.session
  });
});
...
router.get('/confirm-details', function (req, res) {
  ...
  res.render('v1_1/confirm-details', {
    session: req.session
  });
});
...
router.get('/registration-submitted', function (req, res) {
  ...
  res.render('v1_1/registration-submitted', {
    session: req.session
  });
});
